...
void OLED_Num3(unsigned char x,unsigned char y,int number)
{
	...
	{
		OLED_fuhao_write(x,y,11);//fuhaoÊÇÖ¸·ûºÅ¶ø·Ç¸ººÅ£¬ÎÒÕæµÄ·þÁËÕâ¸ö×÷Õß£¬ÓÐµã´ô
		ge = num %10;
		shi = num/10 %10;
		bai = num/100;
		OLED_Num_write(x+3,y,ge);
...
void oled_show(void)
{
		...
		OLED_Num3(14,2,now.v_right);					//ÏÔÊ¾ÓÒ±ßµç»úµÄ±àÂëÆ÷Öµ
		OLED_Num3(4 ,2,now.v_left);					//ÏÔÊ¾×ó±ßµç»úµÄ±àÂëÆ÷Öµ
		...
		{
			case 97:
			OLED_ShowString(0,3,"SR04:",12);
			OLED_Float(3,64,SR04_Distance,2);
			break;
			case 99:
			OLED_ShowString(0,3,"TRACK:",12);
			OLED_Num_write(4,3,C4);					//ÏÔÊ¾ºìÍâ¶ÁÖµ
			OLED_Num_write(5,3,C3);
			OLED_Num_write(6,3,C2);
			OLED_Num_write(7,3,C1);
			break;
		}
}
